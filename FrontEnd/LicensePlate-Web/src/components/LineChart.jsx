export default function LineChart({ data = [] }) {
  ...
  // กันเคส maxValue = 0
  const maxValue = Math.max(1, ...data.flatMap(d => [d.dataset1 ?? 0, d.dataset2 ?? 0]));

  const height = 200;
  const width = 500;
  const padding = 40;

  const denom = Math.max(1, data.length - 1); // กันหารศูนย์เมื่อมี 1 จุด
  const getX = (index) => padding + (index * (width - padding * 2)) / denom;
  const getY = (value) => {
    const v = Number.isFinite(value) ? value : 0;
    return height - padding - (v / maxValue) * (height - padding * 2);
  };

  const points1 = data.map((d, i) => `${getX(i)},${getY(d.dataset1 ?? 0)}`).join(' ');
  const points2 = data.map((d, i) => `${getX(i)},${getY(d.dataset2 ?? 0)}`).join(' ');
  ...
  return (
    <div className="relative">
      <svg width="100%" height={height} viewBox={`0 0 ${width} ${height}`}>
        ...
        {data.map((d, i) => (
          <g key={i}>
            <circle cx={getX(i)} cy={getY(d.dataset1 ?? 0)} r="4" fill="#FCD34D" />
            <circle cx={getX(i)} cy={getY(d.dataset2 ?? 0)} r="4" fill="#93C5FD" />
          </g>
        ))}
